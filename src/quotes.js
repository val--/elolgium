$(document).ready(function(){
 ...
 var skull = $('#skullascii').text();

    // Display a quote right after the page has loaded
    $(function () {
       $('#skullascii').click();
   });

    $('#skullascii').click(randomQuote);

    function randomQuote(evt){

    	var quote = $('#deathcription p').text();
    	var poorguyname = $('#poorguyname').text();
    	...
    	$('#skullascii').html(txt).delay(600).queue(function(next){
    		$(this).html(skull);
    		next();
    	});
    	...
    	for(i = 0 ; i <= sourcelength ; i+=1)
    	{
           ...
           var newpoorguy = quoteSource[randomNumber].name;
           var newDeathYear = quoteSource[randomNumber].deathYear;

           var rapidTimeAnimation = 200;
           var timeAnimation = 500;
           var deathcription = $('#deathcription');
           var poorguyname = $('#poorguyname');

           poorguyname.fadeOut(timeAnimation, function(){
            poorguyname.html('');
            poorguyname.append(''+newpoorguy+' &nbsp;<span id="dateDeath">'+newDeathYear+'</span>');
            poorguyname.fadeIn(timeAnimation);
        });

           deathcription.fadeOut(timeAnimation, function(){
            deathcription.html('');
            deathcription.append('<p>'+newQuoteText+'</p>');
            deathcription.fadeIn(timeAnimation);
        });  
